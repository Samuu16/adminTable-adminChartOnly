...
def create_admin_user():
    ...
    if not existing_admin:
        ...
        db.session.commit()
        get_or_create_level_sensor_data_class(f'level_sensor_data_{admin_name}')  # Create table for super admin
...
def manage_sensor_data(adminname):
    ...
    if request.method == 'GET':
        ...
        with db.engine.connect() as connection:
            result = connection.execute(level_sensor_table.select()).fetchall()
        ...
        for row in data:
            for key, value in row.items():
                if isinstance(value, (datetime, bytes)):
                    row[key] = custom_json_serializer(value)
